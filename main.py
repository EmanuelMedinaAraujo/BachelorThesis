...
from Logging.wandb_loggger import Logger, log_used_device
...
def train_and_test_model(cfg: DictConfig):
    ...
    logger = Logger(dataset_length=cfg.hyperparams.dataset_length, log_in_wandb=cfg.hyperparams.log_in_wandb, cfg=cfg)
    ...
    for epoch_num in range(cfg.hyperparams.epochs):
        train_loop(dataloader=train_dataloader,
                   ...
                   logger=logger,
                   epoch_num=epoch_num,
                   error_tolerance=cfg.hyperparams.tolerable_accuracy_error)
