...
def plot_planar_robot(dh_parameter, standard_size=False, show_title=True):
    # General settings
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    ax.set_xlabel('X')
    ...
    for i in range(dh_parameter.shape[0]):
        ...
        end_coordinates = start_coordinates + np.array([link_length * math.cos(total_angle), link_length * math.sin(total_angle)])
